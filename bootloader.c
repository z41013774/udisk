...
#define STATIC
STATIC void __RdPage512(uint8_t *buf)
{
	unsigned i;
	for (i = 0; i < 512; i++) {
		buf[i] =  NF_RDDATA8();
...
STATIC uint32_t NF8_CheckId(void)
{
	int i;
...
STATIC int Nand_IsBadBlockPage512(uint32_t block)
{
    uint32_t 	Page;
	uint8_t 		BAD;
...
STATIC int Nand_IsBadBlockPage2048(uint32_t block)
{
    uint32_t 	Page;
	uint8_t 		BAD;
...
STATIC int Nand_IsBadBlock(uint32_t block,int bLargeBlock)
{
	switch(bLargeBlock) {
	case 0:
...
STATIC int Nand_ReadSectorPage512(uint32_t sector, uint8_t *buffer)
{
...
STATIC int Nand_ReadSectorPage2048(uint32_t sector, uint8_t *buffer)
{
	uint32_t page, data_addr;
...
STATIC int Nand_ReadSector(uint32_t sector, uint8_t *buffer, int bLargeBlock)
{
	switch(bLargeBlock) {
	case 0:
